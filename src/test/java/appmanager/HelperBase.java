...
//import static appmanager.ApplicationManager.robotType;
...
public class HelperBase {
    ...
    public boolean checkLogInUserHRM(String user) {
        ...
        if (wd != null) {
            if (isElementPresent(By.xpath("//a[contains(text(),'Welcome')]"))) {
                WebElement we = wd.findElement(By.xpath("//a[contains(text(),'Welcome')]"));
                String userLoggged = getText(we);
                if (user.equalsIgnoreCase(userLoggged)) {
                    testStepPassed("User is already logged into the HRM application, Username is -->" + userLoggged);
                    blnLogIn = true;
                }
            }
            if (!blnLogIn) {
    ...
    public boolean checkLogInUserMFS(String user) {
        ApplicationManager app = null;
        app = new ApplicationManager(System.getProperty("browser",reader.get("browser.type")));
        if (reader.get("browser.type").toLowerCase().contains("chrome")) {
            app = new ApplicationManager((System.getProperty("browser", BrowserType.CHROME)));
...
//        return blnLogIn;
    ...
    public static boolean checkLogInInvalidUser(String user) {
        ...
        boolean blnLogIn = false;
        try {
            app.initUrlMFS(user); // Use initUrlHRM
            blnLogIn = true;
        } catch (Exception e) {
            e.printStackTrace();
        }
...
public void selectFromDropDown1(String object, String  strVisibleText){
        try{
            Actions action = new Actions(getWebDriver());
            action.click(getWebElement(object)).build().perform();
            action.moveToElement(getWebElement("//option[normalize-space(text())='"+strVisibleText+"']")).click().build().perform();
...
//    }
    ...
    public void enterTextValueTOInput(String Object,String text,String elementName){
        try{
            WebDriver wd = getWebDriver();
            Actions action = new Actions(wd);
            WebElement weValue = getWebElement(Object);
            action.moveToElement(weValue).click(weValue).doubleClick(weValue).build().perform();
//            String value1 = ""+Object+"//input";
            enterText(weValue,text,elementName);
            sleep(1000);
            testStepPassed("Successfully entered the value-->"+ text + "to the textbox-->"+elementName+" ");
    ...
    public void selectTheProgramType(String ProgramType) {
        ...
        } else if (ProgramType.equalsIgnoreCase("First Hrm Club")) {
...
//    }
    ...
    public String getPaginationText(){
        try{
            ...
            String strText = getText(getWebElement(paginationTotal));
            return strText;
    ...
    public int getPaginationTotalRecordCount(){

            String strText = getPaginationText();
            if(strText != null){
                String[] texts = strText.split(" ");
                return Integer.parseInt(texts[texts.length-1]);
            }else{
    ...
    public int getFirstRecordCount(){
        ...
        if(strText != null){
           ...
           return Integer.parseInt(texts[0]);
            ...
            testStepFailed("unable to get the first record from Pagination text");
            return  -1;
        }
    }
    ...
    public void switchToTabs(int index){
